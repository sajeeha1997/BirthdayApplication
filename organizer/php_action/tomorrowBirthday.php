<?php
			...
			$query = "SELECT userID,firstName,lastName,mobile,userType FROM users where  MONTH(dateOfBirthday)=MONTH(CURRENT_DATE) AND DAY(dateOfBirthday)=DAY(CURRENT_DATE+ INTERVAL 1 day)";
			...
			if($exec){
				...
				{
					echo "No Birthday Tomorrow! ".mysqli_error($con);
				}
			}
			else{
